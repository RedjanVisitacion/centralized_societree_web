...
function getProductImagePath($imagePath) {
    ...
    // If it's a relative path starting with uploads/, it's correct
    if (strpos($imagePath, 'uploads/') === 0) {
        return $imagePath;
    }
    
    // If it doesn't contain a slash, assume it's just the filename
...
$sql = "SELECT * FROM afprotechs_products ORDER BY created_at DESC";
...
function displayPendingProducts(products) {
    ...
    products.forEach(product => {
        ...
        if (product.product_image) {
            // If it's already a data URI or HTTP URL, use it as is
            if (product.product_image.startsWith('data:') || product.product_image.startsWith('http')) {
                imageUrl = product.product_image;
            } else {
                // Assume it's base64 and add data URI prefix (backend should handle this, but just in case)
                imageUrl = 'data:image/jpeg;base64,' + product.product_image;
            }
        }
                ...
                <div class="card h-100 border-warning" style="border-width: 2px;">
                    <div class="card-header bg-warning bg-opacity-10 d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <small class="text-muted">by ${escapeHtml(product.creator_name || product.creator_id)}</small>
                        </div>
